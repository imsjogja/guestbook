...
CREATE INDEX idx_rsvp_event_status ON rsvp_responses(event_id, status);
CREATE INDEX idx_rsvp_responded_at ON rsvp_responses(responded_at);

-- Unique constraint: one RSVP per invitation
CREATE UNIQUE INDEX idx_rsvp_unique_invitation ON rsvp_responses(invitation_id);
